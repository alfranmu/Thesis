...
\end{definition} 
Such a field Xis completely determined by its value at the identity X(e). Conversely for every vector v E Te(G) we can define a (smooth) left-invariant vector field X on G by X(y) = (Ly)*(v). This proves

Lemma 8.1.3. The map X f-+ X(e) is a linear isomorphism between the set of left-invariant vector fields on G and Te(G), the tangent space at the identity.
